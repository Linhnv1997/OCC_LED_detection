    ...
    for fi, f in enumerate(files_5s):
        try:
            ...
            for i, j in enumerate([7]):
                ...
                ax.plot(x, y, marker='.', label="YOLOv5s", linewidth=1, markersize=5)
                #ax[i].set_title(s[j], fontsize=12)
                # ax[0].set_title("AP_50", fontsize=12)
                ax.set_title("AP_50:95", fontsize=12)
    ...
    for fi, f in enumerate(files_5m):
        try:
            ...
            for i, j in enumerate([7]):
                ...
                y = data.values[:500, j]
                # y[y == 0] = np.nan  # don't show zero values
                ax.plot(x, y, marker='.', label="YOLOv5m", linewidth=1, markersize=2)
                #ax[i].set_title(s[j], fontsize=12)
        except Exception as e:
            print(f'Warning: Plotting error for {f}: {e}')        
    for fi, f in enumerate(files_5l):
        try:
            ...
            for i, j in enumerate([7]):
                ...
                y = data.values[:500, j]
                # y[y == 0] = np.nan  # don't show zero values
                ax.plot(x, y, marker='.', label="YOLOv5l", linewidth=1, markersize=2)
                #ax[i].set_title(s[j], fontsize=12)
        except Exception as e:
            print(f'Warning: Plotting error for {f}: {e}')
    for fi, f in enumerate(files_5x):
        try:
            ...
            for i, j in enumerate([7]):
                ...
                y = data.values[:500, j]
                # y[y == 0] = np.nan  # don't show zero values
                ax.plot(x, y, marker='.', label="YOLOv5x", linewidth=1, markersize=2)
                #ax[i].set_title(s[j], fontsize=12)
